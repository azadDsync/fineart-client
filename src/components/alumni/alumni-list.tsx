              ...
              <div className="flex flex-col md:flex-row md:items-start gap-4">
                ...
                <div className="order-2 md:order-1 flex-1 space-y-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <h3 className="font-medium text-base leading-snug break-words">{a.name}</h3>
                    {a.batchYear !== undefined && (
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-accent/20 uppercase tracking-wide whitespace-nowrap">{a.batchYear}</span>
                    )}
                  </div>

                  {a.details && (
                    <p className="text-sm sm:text-xs leading-relaxed text-muted-foreground line-clamp-5 sm:line-clamp-4">{a.details}</p>
                  )}
                  <div className="flex flex-wrap gap-x-2 gap-y-1 text-[11px] uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
                    {a.email && <span className="truncate max-w-full sm:max-w-[12rem]">{a.email}</span>}
