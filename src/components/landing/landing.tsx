        ...
        <div className="relative mx-auto max-w-6xl px-4 py-20 sm:py-24">
          <div className="text-center">
            ...
            <h1 className="heading-display display-hero tracking-tight mt-5">
              Give your arts a <span className="highlight-rose">glow up</span>.
              Meet your new
              <span className="highlight-mint"> creative </span>family.
            </h1>
            <p className="mt-6 lead text-muted-foreground max-w-3xl mx-auto">
              Capture, organize, and elevate your ideas across work, life, and
              leisure.
            </p>
            <div className="mt-10 flex items-center justify-center gap-x-4 ">
              <Button
                ...
                size="lg"
                variant="outline"
                className="border-black shadow-[8px_8px_0px_#000] panel-fill "
              >
                <Link href="/sign-up">
          ...
          <div className="flex flex-col gap-16 lg:flex-row">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/events" className="underline font-bold">
                  View all
                </Link>
              </div>
              {eventsLoading || (!upcoming.length && recentEventsLoading) ? (
                ...
                <ul className="space-y-4">
                  {events.slice(0, 4).map((ev) => (
                    <li
                      key={ev.id}
                      className="group rounded-xl border p-4 hover:bg-[#FFD6BA] transition relative overflow-hidden border-black dark:border-neutral-700/40 bg-neutral-50 dark:bg-neutral-900"
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                          <h3 className="font-medium text-base leading-snug flex items-center gap-2 flex-wrap">
                            <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-accent/20 tracking-wide">
              ...
              {announcementsLoading ? (
                ...
                <ul className="space-y-4">
                  {announcements.slice(0, 3).map((an) => (
                    <li
                      key={an.id}
                      className="group rounded-xl border p-4 hover:bg-[#FFD6BA] transition border-black dark:border-neutral-700/40 bg-neutral-50 dark:bg-neutral-900 "
                    >
                      <h3 className="font-medium text-base leading-snug flex items-center gap-2 flex-wrap">
                        <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-accent/20 tracking-wide">
                          {shortDate(an.createdAt)}
                        </span>
                        {an.title}
                      </h3>
                      {an.message && (
      ...
      <section className="py-14">
        <div className="mx-auto max-w-6xl px-4">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/paintings" className="underline font-bold">
              View all
            </Link>
          </div>
          {paintingsLoading ? (
