...
export const EventsList: React.FC<EventsListProps> = ({
  ...
  const formatTimeRange = (startIso?: string, endIso?: string) => {
    ...
    const startTime =
      s.toString() === "Invalid Date" ? null : timeStr(startIso);
      ...
      {isLoading ? (
        ...
        <ul className="space-y-4">
          {filtered.map((ev) => (
            <li
              key={ev.id}
              className="group rounded-xl border  p-5 hover:bg-[#FFD6BA] transition border-black dark:border-neutral-700/40 bg-neutral-50 dark:bg-neutral-900 "
            >
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex-1">
                  <h3 className="font-medium text-base leading-snug flex flex-wrap items-center gap-2">
                    <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-accent/20 tracking-wide uppercase">
                  ...
                  <div className="mt-2 flex flex-wrap gap-3 text-[11px] uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
                    ...
                    {ev.attendeesCount !== undefined && (
                      <span>{ev.attendeesCount} going</span>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-2 self-start sm:self-auto">
                  <button className="text-xs px-3 py-1.5 rounded-md border bg-background hover:cursor-pointer transition">
                    Details
                  </button>
                </div>
              </div>
