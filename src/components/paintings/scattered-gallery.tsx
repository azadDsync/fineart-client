...
export const ScatteredGallery: React.FC<ScatteredGalleryProps> = ({
  ...
  const cardLayouts = useMemo(() => {
    ...
    const results = source.map((item, idx) => {
      ...
      if (idx < initialVisible) {
        ...
        for (let a = 0; a < attempts; a++) {
          ...
          if (ok) { x = candidateX; y = candidateY; break; }
          if (a === attempts - 1) { x = candidateX; y = candidateY; } // fallback
        }
      }
      placed.push({ x, y });
    ...
    return results;
  }, [filteredItems, items.length, size, cardWidth, cardHeight, minDistance, initialVisible]);
  ...
  const visibleCards = useMemo(() => {
    ...
    const pad = 400;
    const view = { left: -offset.x - pad, top: -offset.y - pad, right: -offset.x + vp.width + pad, bottom: -offset.y + vp.height + pad };
    return cardLayouts.filter(c => c.x + cardWidth > view.left && c.x < view.right && c.y + cardHeight > view.top && c.y < view.bottom);
    ...
    const step = () => {
      if (!dragging.current) {
        ...
        if (Math.abs(velocity.current.x) < 0.3 && Math.abs(velocity.current.y) < 0.3) {
          ...
          setOffset(o => clampOffset(o.x + velocity.current.x, o.y + velocity.current.y));
        }
      }
  ...
  return (
    <div className={clsx("relative w-full", className)}>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 text-sm">
          <button
            ...
            className={clsx("rounded-md px-3 py-1.5 border text-xs font-medium", isScatter ? "bg-foreground text-background" : "bg-background")}
          >Scatter</button>
          <button
      ...
      {isScatter ? (
        <div className={clsx("relative h-[calc(100vh-11rem)] w-full overflow-hidden rounded-md border ", className)} ref={viewportRef}
          ...
          >
            <div style={{ width: size, height: size, position: 'relative' }}>
              {visibleCards.map(card => (
                <GalleryCard key={card.id} card={card} width={cardWidth} height={cardHeight} />
              ))}
            </div>
          </div>
          <div className="pointer-events-none absolute inset-x-0 top-0 flex justify-between p-4 text-xs text-muted-foreground">
            <span>Drag to explore</span>
            <span>{visibleCards.length} / {filteredItems.length} visible</span>
          </div>
        </div>
      ) : (
...
interface CardProps {
  card: ReturnType<typeof Object.assign> & { id: string; title: string; subtitle?: string; imageUrl: string; x: number; y: number; rotation: number };
